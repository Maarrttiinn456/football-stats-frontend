...
import { columnDefAttack } from './teamColumnsAttack';

type TeamStatsShotsCardProps = {
    data: TeamWithStatistics[];
};

const TeamStatsAttackCard = ({ data }: TeamStatsShotsCardProps) => {
    const [sorting, setSorting] = useState<SortingState>([
        { id: 'TotalAttack', desc: true },
    ]);

    // eslint-disable-next-line react-hooks/incompatible-library
    const table = useReactTable({
        data: data,
        columns: columnDefAttack,
        state: {
            sorting,
        },
...
export default TeamStatsAttackCard;
